...
output <- SS_output(dir = dirSS3output, repfile = "Report.sso", compfile = "CompReport.sso", covarfile = "covar.sso", ncols = 200, forecast = TRUE, warn = TRUE,covar = FALSE, checkcor = TRUE, cormax = 0.95, cormin = 0.01,printhighcor = 10, printlowcor = 10, verbose = TRUE,printstats = TRUE, hidewarn = FALSE, NoCompOK = FALSE,aalmaxbinrange=0)

# Changes from previos version: 
# year is now called "Yr"
...
# STOCK WEIGHT-AT-AGE: 
data@stock.wt <-FLQuant(dimnames = list(age = agefirst:agelast, year = yearfirst:yearlast, unit = 'unique', season = 'all', area = 'unique'), quant = "age", rep(as.numeric(unlist(t(output$wtatage[(output$wtatage$Fleet==0)&(output$wtatage$Sex==1),-c(1:6,dim(output$wtatage)[2])]))), nyears), units = 'kg')
...
# Catch number at age: 
data@catch.n <- FLQuant(dimnames = list(age = agefirst:agelast, year = yearfirst:yearlast, unit = 'unique', season = 'all', area = 'unique'),
                        quant = "age", 
                        natage*(1-exp(-fatage-matage))*fatage/(fatage+matage), units = '10^3')
...
if( output$nfishfleets == 1){
  wtatage_commercial <- t(output$wtatage[(output$wtatage$Fleet==1)&(output$wtatage$Sex==1)&(output$wtatage$Yr<=yearlast),-c(1:6,dim(output$wtatage)[2])])
}
