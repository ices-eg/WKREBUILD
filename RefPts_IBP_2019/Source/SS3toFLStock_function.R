fSS3toFLStock <- function (dirSS3output, stockname, fbar_amin, fbar_amax) {
...
output <- SS_output(dir = dirSS3output, repfile = "Report.sso", compfile = "CompReport.sso", 
                    covarfile = "covar.sso", ncols = 200, forecast = TRUE, warn = TRUE,
                    covar = FALSE, checkcor = TRUE, cormax = 0.95, cormin = 0.01,
                    printhighcor = 10, printlowcor = 10, verbose = TRUE,printstats = TRUE, 
                    hidewarn = FALSE, NoCompOK = FALSE, aalmaxbinrange=0)

###############################################################
####  CREATING FLSTOCK FROM EXTERNAL FILES   ##################
#### This is a MOCK FLSTOCK object, most of the slots have 1111 and the contents 
#### of other slots have been suitably modified!!! 
#### The intention here is to have an FLStock object which contains the required 
#### information to fit the Stock-Recruitment relationship in EqSim -- this FLStock 
#### object should not be used for any other purpose!!!
...
auxi <-  output$natage[(output$natage$"Beg/Mid" == 'B')&(output$natage$"Sex" == 1),]
...
if(output$nsexes==2){
  ...
  auxi <-  output$natage[(output$natage$"Beg/Mid" == 'B')&(output$natage$"Sex" == 2),]
...
# STOCK WEIGHT-AT-AGE: 
#data@stock.wt <-FLQuant(dimnames = list(age = agefirst:agelast, year = yearfirst:yearlast, unit = 'unique', season = 'all', area = 'unique'), 
#                        quant = "age", rep(as.numeric(unlist(t(output$wtatage[(output$wtatage$fleet==0)&(output$wtatage$gender==1),-(1:6)]))), nyears), units = 'kg')
#data@stock.wt <-FLQuant(dimnames = list(age = agefirst:agelast, year = yearfirst:yearlast, unit = 'unique', season = 'all', area = 'unique'), 
#                        quant = "age", rep(as.numeric(unlist(t(output$wtatage[(output$wtatage$Fleet==0)&(output$wtatage$Sex==1),-(1:6)]))), nyears), units = 'kg')

#stock weights taken from wtatage.ss_new
stkwt <- c(0.000411028,0.0143499,0.0395859,0.0663188,0.0963278,0.127432,0.157989,0.186891,0.213478,0.237427,0.258656,0.27724,0.293347,0.307197,0.319032,0.329093,0.337611,0.344796,0.350842,0.355918,0.364556)
data@stock.wt <- FLQuant(dimnames = list(age = agefirst:agelast, year = yearfirst:yearlast, unit = 'unique', season = 'all', area = 'unique'),
                         quant = "age", stkwt, units = 'kg')
...
selex <- selex[(selex$Fleet<= output$nfishfleets)&(selex$Factor=="Lsel")&(selex$Yr >= yearfirst)&(selex$Sex==1), ]
dim(selex)
names(selex)
...
if(output$nfishfleets == 1){
  ...
  wtatage_commercial <- t(output$wtatage[(output$wtatage$Fleet==1)&(output$wtatage$Sex==1),-(1:6)])
}
...
#taken from atageat.ss_new
wtatage_commercial <- c(0.0068828,0.0370763,0.0632537,0.0926677,0.123027,0.152688,0.18067,0.206467,0.229808,0.250563,0.268728,0.284414,0.297815,0.309169,0.318727,0.326731,0.333408,0.33896,0.343566,0.347378,0.353556) 

data@catch.wt <- FLQuant(dimnames = list(age = agefirst:agelast, year = yearfirst:yearlast, unit = 'unique', season = 'all', area = 'unique'),
                         quant = "age", wtatage_commercial, units = 'kg')
...
#not interested in the rest of this...just comment it out for now
